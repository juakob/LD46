...
Parser.prototype = {
	...
	,parseRegular: function(line) {
		...
		if(this.matchParent.match(line)) {
			...
			if((__map_reserved[parent] != null ? _this.getReserved(parent) : _this.h[parent]) == null) {
				if(Parser.logSkips) {
					console.log("src/Parser.hx:131:","Skip " + this.currentClass.name + " parent " + parent);
				}
				return;
			}
		...
		if(StringTools.endsWith(line,".prototype = {") || line.indexOf(".prototype = $extend(") != -1) {
			if(Parser.logTypes) {
				console.log("src/Parser.hx:123:","" + this.currentClass.name + " {");
			}
			this.mode = 1;
			return;
		}
		...
		if(this.matchStaticVar.match(line)) {
			...
			if(Parser.logTypes) {
				console.log("src/Parser.hx:123:",s);
			}
		...
		if(this.matchStaticArr.match(line)) {
			...
			if(Parser.logTypes) {
				console.log("src/Parser.hx:123:","" + className1 + "." + field1 + " = " + value1);
			}
		...
		if(this.matchStaticFunc.match(line)) {
			...
			if(Parser.logTypes) {
				console.log("src/Parser.hx:123:",s1);
			}
			if(Parser.logBodies) {
				console.log("src/Parser.hx:127:",body);
			}
	...
	,setConstructor: function(name,args) {
		...
		if(Parser.logBodies) {
			console.log("src/Parser.hx:127:",constructor.body);
		}
		var _this = this.classes;
	...
	,setObj: function(name) {
		...
		if(Parser.logTypes) {
			console.log("src/Parser.hx:123:","Class " + name + " {}");
		}
		var _this = this.classes;
		if(__map_reserved[name] != null) {
			_this.setReserved(name,klass);
	...
	,setNameId: function(name,nameId) {
		...
		if((__map_reserved[name] != null ? _this.getReserved(name) : _this.h[name]) == null) {
			if(Parser.logSkips) {
				console.log("src/Parser.hx:131:","Skip " + name + " id " + nameId);
			}
			return;
		}
		var _this1 = this.classes;
	...
	,compareClass: function(klass,result) {
		...
		if(old == null) {
			console.log("src/Parser.hx:340:","New class: " + className);
			result.push({ type : "addClass", klass : klass});
			return;
		}
		if(old.parent != klass.parent) {
			console.log("src/Parser.hx:345:","" + className + ": new parent: " + klass.parent);
			result.push({ type : "addClass", klass : klass});
			return;
		}
		...
		while(_g < keys.length) {
			...
			if((__map_reserved[key1] != null ? _this1.getReserved(key1) : _this1.h[key1]) != newValue) {
				console.log("src/Parser.hx:354:","" + className + ": static var " + key1 + " value: " + this.minString(newValue));
		...
		while(_g1 < keys1.length) {
			...
			if("" + Std.string(value) != "" + Std.string(newValue1)) {
				console.log("src/Parser.hx:363:","" + className + ": func " + key2 + "() value: " + this.minString("" + Std.string(newValue1)));
				if(newValue1 == null) {
	...
	,compareEnum: function(en,result) {
		...
		var old = __map_reserved[key] != null ? _this.getReserved(key) : _this.h[key];
		if(old == null) {
			console.log("src/Parser.hx:380:","New enum: " + en.nameId);
			result.push({ type : "addEnum", enumeration : en});
			return;
		}
		if(old.body != en.body) {
			console.log("src/Parser.hx:385:","New enum body: " + this.minString(en.body));
			result.push({ type : "addEnum", enumeration : en});
			return;
		}
	}
